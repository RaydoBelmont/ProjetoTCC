...
import BotaoRelatorioChamadosPorCliente from "../btnGerarRelChamadosPorCliente";
...
export default function ModalRelChamadosPorCliente({
  isOpen,
  setModalOpen,
  listaClientes,
  listaChamados,
  ...
  const [termoPesquisaCliente, setTermoPesquisaCliente] = useState<string>("");
  ...
  const [isClienteDropdownOpen, setClienteDropdownOpen] = useState(false);
  const statusDropdownRef = useRef(null);
  const clienteDropdownRef = useRef(null);

  useClickAway(statusDropdownRef, () => setStatusDropdownOpen(false));
  useClickAway(clienteDropdownRef, () => setClienteDropdownOpen(false));
  ...
  const clientesFiltrados = listaClientes.filter((cliente) =>
    cliente.nome.toLowerCase().includes(termoPesquisaCliente.toLowerCase())
  );
    ...
    >
      <Card className="mx-auto w-full max bg-[#384152]">
        <CardBody className="flex flex-col gap-4">
          ...
          {/* Select customizado para Cliente com pesquisa */}
          <div className="flex items-center mt-4">
            <strong className="text-white font-bold text-lg mr-2">
              Cliente:
            </strong>
            <div ref={clienteDropdownRef} className="relative w-full">
              <button
                onClick={() => setClienteDropdownOpen(!isClienteDropdownOpen)}
                className="w-full bg-white text-black p-2 mt-2 rounded cursor-pointer"
              >
                {clienteSelecionado
              ...
              {isClienteDropdownOpen && (
                <div className="absolute w-full bg-white text-black rounded mt-2 z-10">
                  {/* Campo de pesquisa */}
                  <input
                    type="text"
                    placeholder="Pesquisar cliente..."
                    value={termoPesquisaCliente}
                    onChange={(e) => setTermoPesquisaCliente(e.target.value)}
        ...
        <CardFooter className="pt-0 flex justify-end space-x-2">
          <BotaoRelatorioChamadosPorCliente
